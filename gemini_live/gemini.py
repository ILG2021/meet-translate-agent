...
class GeminiMultimodalLiveLLMService(LLMService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._prev_session_handle = None
        ...
        try:
            ...
            config_data = {
                "setup": {
                    ...
                    "session_resumption": {
                        "handle": self._prev_session_handle
                    }
                }
            }
    ...
    async def _receive_task_handler(self):
        async for message in self._websocket:
            ...
            elif evt.usageMetadata:
                logger.info( f'Used {evt.usageMetadata.totalTokenCount} tokens in total.')
            elif evt.sessionResumptionUpdate:
                if evt.sessionResumptionUpdate.resumable and evt.sessionResumptionUpdate.newHandle:
                    self._prev_session_handle = evt.sessionResumptionUpdate.newHandle
                    logger.info(f"Resumed session update with handle: {evt.sessionResumptionUpdate.newHandle}")
